...
-   Now that the RE model is working, my next steps are:
    ...
    -   Write equation for relative error metric, and run that by Mark and Cecilia to check it (this week or next week)
...
-   Ran the rest of the subregions for both species, and the eastern/sebastes_polyspinis has 0 values for catch, which makes the CV an NA, and therefore the RE model can't be calculated - check with Mark or Cecilia about how to deal with this.
...
$$
MRE = \frac{1}{N}\sum_{n = 1}^N \frac{P_n}{O_n - 1}
$$

-   Not sure how to quantify "variability of these relative errors" maybe the distribution of this $N$ length vector of values? Variance, etc.
