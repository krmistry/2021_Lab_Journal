...
### Model comparison notes & troubleshooting
...
### Resampling study notes & troubleshooting

#### 7/26/21

-   The meeting on 7/22 didn't end up happening, it was postponed to 8/5, so do the above steps before then

-   Today, I start working with Mark on starting to conceptualize the first simulation experiment

    -   From my proposal, this is what I said about that simulation experiment: "I will conduct a simulation study to determine whether the model outputs from the two models (such as unfished biomass, $B_0$), which will be used to set management rules, differ enough between the delta-GLMM and random walk model to produce different TAC at the subregional level. For the simulations, I will create 100 simulated data sets using the estimated variance parameters for the random walk model, and another 100 simulated data sets using the estimated variance parameters of the random effects and the mean and variance of the fixed effects from the delta-GLMM model. Each simulated data set will have the same annual sample size as the original data set. I will then fit the two models to each of the simulated data sets and assess how well each model fits in terms of accuracy and precision. The specific metrics used will be determined as the project develops."

    -   From 6/23 meeting with Jim and Paul: Paul thinks that I really need a simulated data set to properly evaluate performance metrics (which is what I am planning based on above description, right? Check with Mark). I also need to think about how to link model performance with management decisions, like how much area apportionment changes depending on results from the two models.

    -   Notes from meeting with Mark:

        -   For a randomly distributed simulated data set: X \~ Unif(0, max x) and Y \~ Unif(0, max y). The characteristics of a random distribution are that the nearest neighbor distance is Poisson distributed

        -   Look at how much the CV varies for the one where catch = 0 (so CV won't be NA and I can run the RE model) I could take that data point out, but since its the most recent year, I don't want to do that if I don't have to. Ask Cecilia (Mark didn't really answer this question).

#### 8/5/21

-   Met with Jim and Paul to give them an update on my progress. Here is the [powerpoint.](https://docs.google.com/presentation/d/1sdyE6PQ926CZ0QCQ4y80QFfg433lugK7Y_364pxWvXs/edit?usp=sharing)

    -   Another NOAA group is working on POP in VAST

        -   Recent years have a discrepancy between VAST model results and survey data (something to be aware of)

    -   I asked about the difference between the Index_ctl and Table_for_SS3.csv in terms of which I should use; the Table_for_SS3 data is the correct one to use when comparing with RE for how accurately the model results correspond to the survey data

    -   "Proportion by area" might be a better way to judge model performance; not sure exactly what this entails (check with Mark or Cecilia)

    -   Paul is curious about why the RE model results adhere to exactly to the survey data (I am also curious about this :D )

        -   Paul volunteers to look at my RE code

        -   Jim suggests that I could experiment a bit more with this too

    -   Find out units of biomiass (I should know this anyway)

    -   First MRE equation is right according to Paul

    -   Consider using either jackknife or bootstrap methods (look up jackknife) either in addition to or instead of the simulation study

    -   For future update meetings, make abstracts to attach to each calendar meet

    -   Potential issue to consider: if VAST is the operating model for the simulation study, then the VAST results may have an advantage. However, it may be interesting to see how well the RE model performs in this case.

#### 8/31/21

-   Met with Cecilia, updated her on my progress and what Jim and Paul had to say in the 8/5 meeting.

    -   She agreed that the proportion of area was a good idea, and that it was a pretty simple calculation

    -   She also agreed that using a bootstrap or jackknife resampling method instead of a simulation study might make more sense for evaluating VAST vs. RE model performance, in other words instead of the first simulation study. She did encourage me to think about it and come to my own conclusions about which would make more sense.

        -   I agree that resampling rather than simulation makes more sense for this type of model evaluation, so now I have to decide whether to use bootstrap or jackknife. Jackknife has the advantage of being fully deterministic rather than random (which bootstrap is), making it both reproducible and easy to see which data points might have leverage compared to others.

    -   Another option for model evaluation is CV, which is a metric that measures the amount of variation, definitely something that we supposedly care about for this study. So I'll look into that too, as well as see if there are other metrics to quantify variation that should be considered.

    -   Finally, I told her I would email Paul my RE code for his review, as we all agree that the RE results don't look right since they so closely adhere to the survey estimates in the survey years. It took me longer than it should have to clean up my code and send it to him, but I finally did on Tuesday 9/7.

        -   He may not get back to me very quickly though, so I need to get on figuring out how the RE model works to see if I can figure it out.

### 9/13/2021

-   Over the past two weeks, I figured out what was going wrong in the CV calculation in the RE code (I was multiplying by 100 as thought it was a percent, when it wasn't), and re-ran the RE models, and the results look much more plausible (they don't adhere so closely to the survey data).

    -   The exception is eastern NR, which had the 0 value in its data. I made the 0 into 0.01, but variance for that data point was also 0, meaning that the CV was 0 (instead of NA, which is what it was before) and this meant that the model failed to run due to an error with the inverse matrix not existing. Email Cecilia today to see if she has a suggestion for how to handle this.

-   For the meeting with everyone on Wednesday, my to do list is:

    -   Write an abstract of my current progress and immediate next steps

    -   Create slides for:

        -   The re-run RE result plots

        -   Laying out all of the metrics that have been proposed so far, grouped based on what they measure (precision, quantity of variance, etc.) and my preferences for which ones to use:

            -   MASE and MRE both measure accuracy the model's accuracy to the survey data

            -   Proportion of area measures how the total biomass is distributed between the subregions; would this be compared to the survey data POA? Probably, I guess. Based on past conversations, I'm leaning towards this rather than either MASE or MRE.

            -   CV and proportional variability (PV) measure the amount of variance (which doesn't need to be compared to the survey data, I don't think), and PV is my preference.

                -   Percent deviance is a new metric that Cecilia mentioned today, which is now an automatic output for VAST and theoretically wouldn't be difficult to implement for the RE; it measures the amount of variance explained by each covariate compared to the null model. So a little bit different from PV or CV, I think, but somewhat related.

            -   Maybe run the data for each of these metrics, or at least the ones I think I want to do and present that too...

        -   Other updates and questions:

            -   I think I will use the jackknife method over the bootstrap, both for ease of computing and reproducibility, but ask if there is any issues with having a relatively small sample size (14 survey years or 16 survey years, depending on species)

                -   Some studies found that the jackknife was better with small sample sizes than bootstrap, others found the opposite and I'm not sure which would work best for this instance (the studies had fairly different types of questions and data scenarios)

            -   Ask Paul if he knows what the functions step() and obs() and the conditions mceval_phase() and sd_phase() are in the re.tpl file, or where I can find a definition

        -   Next steps:

            -   Make final decision about which metrics to use, and whether to use jackknife or bootstrap

            -   Implement resampling method (for VAST at least, will likely use Hyak or the QERM server as it will take a long time to run on my computer)

            -   WRITE UP METHODS - for REAL this time

### 9/14/21

-   Meeting with Mark notes:

    -   Update CV for Pierce award nomination (completed on 9/20)

    -   Ideas for places to present:

        -   Alaska Marine Science Symposium (<https://alaskamarinescience.org/abstracts> - **abstract submission closes on 10/15** - get on this!!!)

        -   AFS 2022 - will be held in August in Spokane

    -   Ask about code review among lab mates (did this with VAST code, and Markus and Mark gave me notes, which still need to be incorporated)

    -   Job shadowing might be an option - talk with Mark more about this soon

    -   Hyak access might be a good lab meeting topic (ask John Best for help)

### 9/15/21

-   Meeting with Jim, Paul, Cecilia and Mark notes:

    -   Cecilia took notes [here](https://docs.google.com/document/d/1eeHyZe7HVil2C7LjcdQLTQIvaj6xl0y8z7bcbIlMs1I/edit?usp=sharing)

    -   My notes:

        -   Have we considered a MARSS model? As kind of an intermediate step between RE and VAST

            -   Mark says the code for this would be pretty simple

            -   Paul says he thinks its a good idea (Jim proposed it)

        -   Paul comments seeing smaller variances in survey years is a common thing for RE modle, may be good to explore why this is

        -   Attend Plan Team meeting next week for Cole's presentation about the RE model (I did, and this is the relevant [presentation slides](https://meetings.npfmc.org/CommentReview/DownloadFile?p=02281578-6fca-4f7b-a33b-0f129152a7e4.pdf&fileName=PRESENTATION_Tier%204%20and%205%20Consideration.pdf) and [associated paper](https://meetings.npfmc.org/CommentReview/DownloadFile?p=86098951-a0ed-4021-a4e1-95abe5a357fe.pdf&fileName=Tiers%204%20and%205%20assessment%20considerations.pdf))

        -   Use proportion of area instead of biomass estimate and then perform metrics on that, things like MASE and MRE (I think this is what Jim was talking about)

        -   Jackknife seems to have consensus at he best option to start with

        -   Email Paul about re.tpl functions, to make sure the one I'm using is the right one (sent him the email on 9/21, haven't heard back yet as of 9/30)

        -   Start sending update slides with results as I have them rather than waiting until the next meeting

        -   Start email thread with everyone about survey boat opportunity (STILL NEED TO DO THIS)

### Between 9/15 and 9/29

-   I sent my VAST code to both Markus and Mark asking for feedback; got responses, still need to implement them

-   I worked on streamlining and cleaning up my RE code in preparation for doing jackknife resampling, and while doing this ran into an issue with compile_admb(), where it was giving me an error saying that it couldn't find the re.tpl file at the file path I provided

    -   After trying a lot of different versions, it really seems to sometimes work and sometimes not, with the exact same file path inputs. I went to create a github issue about it, but there was a somewhat similar issue open from 2017, which had never been responded to or closed, so I may not bother.

### 9/29/21

I need to get MUCH better at writing notes here. New goal: write weekly updates, at the very least, and ideally write at least a brief description of activities for each day

-   Meeting with Mark notes:

    -   MARSS models have been used for this type of question, so it makes sense that managers would want to know if VAST is better than this method, or at least how it compares in performance

    -   May be worth experimenting a bit to see if this is even a good idea - check how many unique sites I have; if there are too many sites, it may not be worth it because it would take to long to compute

        -   Check on 9/30, and for POP there are 9,893 total sites across years, with a minimum of 489 and maximum of 837 in sites for each year; Mark says that just 489 sites would be pretty computationally intensive.

            -   However, I only looked at unique site combinations; if I look for sites grouped within a certain distance from each other and treat those as the same, maybe the number of sites would be reduced enough to consider it? Something to experiment with...

    -   Contact Cole about compile_admb() issue - nevermind, figured it out

    -   For jackknife, make the missing years data NA; check if this breaks the RE model - it does; had to just remove the year completely

    -   Plan for next steps:

        -   Incorporate feedback from Mark and Markus on VAST code

        -   Try running RE model with NAs for one of the years (Mark's suggested method for the jackknife), to see if it breaks - I think it might, so the other option is to completely omit a year, I'm pretty sure.

        -   VAST might be able to handle NAs for a year, I'll have to check that too though. Mark thinks it should be fine. I should probably do that before I get access to Hyak anyway, so I can do that while I'm waiting to be given access.

            -   10/4: I just received an email that I've been granted access to Hyak, so can move forward with this imminently

        -   Write an abstract to submit for [Alaska Marine Sciences Symposium](https://alaskamarinescience.org/home) - deadline is OCTOBER 15! (Also, check with Cecilia if she thinks this is a good idea, but I think it will be) Can use the one I wrote for Jim & Paul as a starting point.

            -   Looks like the conference will be virtual - dates are 1/25 - 1/27.

            -   I'll also need an abstract for the USGS Coop student presentations, although not sure when - this can probably be the same abstract.

        -   EMAIL CECILIA - really need to do this, both to give her an update on what I've been doing in the last 2 weeks, and to ask her opinion on the MARSS model idea. And for speaking opportunities.

        -   Set a weekly writing goal and start doing it - maybe at least 1 page per week? See how that goes. It would be good to look at some other theses to see how long they are, how different from a published paper (which I assume is shorter?).

        -   EMAIL COMMITTEE MEMBERS - need to set up a committee meeting sometime this quarter, to update Sarah and I guess get approval for my annual review stuff (?)

### 10/11/21

Not a great start for the weekly updates, but oh well.

### 10/12/21

-   Meeting with Mark notes:

    -   Contact Cole Monnahan to ask about how he added to the 0 values (to all values, or just the ones with 0)

    -   Conference abstracts need to be approved through Digital Measures (by Sarah, or maybe someone else), possibly BEFORE being submitted to the conference (so for the Alaska Marine Science Symposium, I need to do this ASAP)
...
-   Goals for this week:
    ...
    -   Submit abstract to Alaska Marine Science Symposium by 10/15 at noon - the first step is getting the abstract approved on Digital Measures, either by Mark or Sarah

        -   10/13: I've submitted it to Digital Measures for review, so just waiting for approval
        -   10/14: Mark sent some edits, so I incorporated them and resubmitted. Waiting for approval again.
        -   10/15: Approval didn't come through, but Mark told me to submit it anyway, so it's been submitted. I'll hear in mid-November if it's accepted.

    -   Login to Hyak, and navigate around to start familiarizing myself

        -   10/13: I've worked through all of the steps [here](http://depts.washington.edu/uwrcc/getting-started-2/getting-started/), and now just have to wait an hour to see if the Hyak and Lolo servers appear in my active subscriptions; once they do, I can try logging in :D

            -   It doesn't seem to be working (says "no record matched" when I try to subscribe), so I posted in the RCC slack to see if anyone had suggestions

            -   10/14: tried again today, and for some reason it worked. Not complaining though, yay. Now I can try logging in.

                -   It worked! I now have access to Mox on Hyak :D :D :D :D

    -   Start working through terminal [command training materials](https://swcarpentry.github.io/shell-novice/)

    -   Work on writing methods section - at least 1 - 2 hours, more would be ideal

        -   10/15: worked on continuing to flesh out the outline

    -   Start incorporating edits from Mark's code review for the VAST code, and getting the code set up to run in Hyak (John's suggestion is to use for loops)

        -   It would be great to get this mostly done by next week, so I can maybe consider actually running it by the end of next week (maybe)
        -   10/14: incorporated all of Mark and Markus's comments, now the next step is to set up the code to loop through the resampled data

    -   Submit abstract to GSS

        -   10/13: COMPLETED!!!!!

    -   Start working on powerpoint for presentations - practice presentation for the lab will be next Wednesday, 10/20

-   Presentation sequence and run times:

    -   USGS Coop student symposium - Wednesday, 10/27 for 8 minutes (2 minutes for questions)

        -   Definitely happening, no abstract needed just a title

    -   GSS - Thursday, 11/4 for 12 minutes (3 minutes for questions)

        -   Submitted abstract, but very likely to be accepted

    -   Alaska Marine Science Symposium - 1/25 - 27, 10 - 15 minutes

        -   Haven't submitted abstract, unknown likelihood of acceptance, and it could be either a poster or oral presentation
        -   10/15: submitted abstract, requested oral presentation preferentially

### 10/14/21

-   Goals for week of 10/18:

    -   Build R on Mox per John's instructions

### 10/15/21

-   Notes from meeting with Cecilia

    -   Proportion of area is what I will transform all of the results into before performing metrics

    -   She's fine with the plan to leave the MARSS model until the end, and if I have time to do it then great but if not then just leave it as a discussion item for future research

    -   She's on board with my plan to do the simulation study if I have time (MARSS will be behind that priority wise)

    -   Ask Paul about how to predict with the random walk model

-   I was watching Dan Ovando's quantitative seminar about predictive modeling, and was reminded that the gold standard for predictive modeling is to separate the data into test and train parts (20:80 is the typically used proportion), then from the train data, resample it and for each resample dataset, separate again into test and train data. I haven't done this with my data, and I probably should, I think? Email Mark and/or Cecilia about it.

    -   In the meantime, maybe try it out with the VAST data and see how hard it is to separate - do I need to make sure that I have an even distribution between years and subregions? It seems like I might need to be careful about that...

### 10/21/2021

-   I've been mostly working on presentation slides this week, but as part of that I finally made graphics of the VAST initial results compared to the survey data, and it definitely looked weird. The bias corrected results, from Table_for_SS3, were 0 for the non-survey years, which definitely isn't right.

    -   So, I reran the VAST model (the previous run was from back in early August) to see if the issue persists.

        -   This message showed after bias correction finished (not sure if its important):

            -   Warning messages:

            -   1: In sparseMatrix(i = index.i, j = index.j, x = Aij, dims = c(mesh\$n, : 'giveCsparse' has been deprecated; setting 'repr = "T"' for you

            -   2: In sparseMatrix(i = index.i, j = index.j, x = Aij, dims = c(mesh\$n, : 'giveCsparse' has been deprecated; setting 'repr = "T"' for you

            -   3: In sparseMatrix(i = index.i, j = index.j, x = Aij, dims = c(mesh\$n, : 'giveCsparse' has been deprecated; setting 'repr = "T"' for you

            -   4: In na.omit(as.numeric(strsplit(part, "")[[1]])) : NAs introduced by coercion

        -   It produced the same results - 0 for non-survey years in Table_for_SS3

    -   I uploaded the scripts to github, so Cecilia can try it out

### 10/25/21

-   Priorities for this week:

    -   Finish presentation slides before Wednesday - done

    -   Write research description for website

    -   Build R on Hyak

        -   it would be nice to run VAST once this week to try out the process, but this is low priority

    -   If Cecilia hasn't gotten back to me by Wednesday check in with her about VAST implementation - she got back to me on Wednesday, and says that the model code looks good, so these results may be accurate

    -   Convert the random walk resampled results to proportion of area, and run all of the proposed metrics by Thursday's meeting with collaborators - completed POA transformation with RE all data, VAST all data and RE resampled data, and ran MASE and MRE calculations and was able to show result plots at collaborators meeting on Thursday

        -   Create plots with these results and put them in slides before the meeting

        -   Also, update my abstract before Thursday - I don't think there's much to add, other than correcting how proportion of area is being used - done

        -   There's nothing to compare them to currently, but I can at least try again to get real feedback about which metrics should be used

            -   Actually, I could use the probably wonky VAST results, just to have something to compare (and to see exactly how wonky they are) - done

    -   Ask Mark about test and train data in our meeting on Wednesday, and if it's something that I should have already done - the jackknife resampling is basically doing the same thing, so I don't need to do this at the beginning, and it doesn't make sense for this type of model anyway, which will be rerun every time there is new data to add

    -   Write resampling code for VAST, so that at least is ready to go when I'm confident that the implementation is correct

### 10/26/21

-   I've been working on transforming the predicted biomass from the resampled datasets into proportion of area, and I'm running into an issue with the resampled dataset where 2001 is excluded, because there was no 2001 survey in the eastern subregion, for either species. Since I need estimates from all subregions in order to calculate a proportion of area, I think the only solution for this issue is to exclude those resampled datasets. Double-check this decision with Cecilia and/or Mark.

-   Watching Madison Hall's groundfish presentation today - she is working on rockfish in the GOA, and probably has some good resources for performance of bottom trawl surveys (not great performance for rockfish), which should reinforce why VAST might be better than RE, even if there isn't a huge increase

    -   Madison presented some VAST results, it looks like from Curry in 2018

    -   The bottom trawl survey is stratified by depth - there is a minimum of 2 sites for each dept strata (this was unclear to me before)

    -   She has done simulations of survey data - I should definitely contact her and ask if she can give me advice (and maybe her code) on how to do the spatial distribution simulation study I want to do

    -   Look up Jones et al. 2021 - looked at habitat affinity for several species, including POP and NR

### 10/28/21

-   Questions/updates for collaborator meeting:

    -   Update: completed RE resampling with jackknife method, VAST resampling still needs to be done

    -   Review proportion of area plots, and verify that this is the transformation that Jim had in mind, and that this is what I should be using when calculating metrics

    -   Review metric plots that are completed so far (MASE and MRE)

        -   Try to get them to commit to a recommendation of which to use, or if there's a different one that would be better for measuring accuracy

    -   Next step is to calculate CV and PV and compare those as metrics; I'll post them in the drive for review

    -   Address idea to add in MARSS model to analysis - basically, I still want to try to do a simulation study of how different distributions are captured by the models, and time-wise I would like to prioritize that. If I can add in a MARSS model on top of that, then I will, but it might have to just be in my discussion as a good next step for someone to do.

-   Meeting notes from collaborators meeting (Jim, Paul and Cecilia)
...
-   Basically, know what your end goal is (i.e. exploration, inference or prediction) before deciding on model selection. For the NOAA groundfish subregion project, my goal is definitely prediction

-   Future articles to read, regarding developing methods for inference that account for model selection and associated uncertainty:

    -   Selective inference - Taylor and Tibshirani 2015

    -   Stability selection - Meinshausen and Buhlmann 2010

    -   Other methods - Shen et al. 2004, Lockhart et al. 2014, G'Sell et al. 2016

-   "...researcher degrees of freedom: all of the implicit choices researchers make that influence an analysis (Simmons et al. 2011)" pg 3 - GREAT concept, and term. Definitely going to remember that one.

-   Interesting point: understanding more about the system will most likely improve predictive models, but the best predictive model may not be all that informative for inference (may not increase understanding) - this is because it doesn't matter how little a covariate adds to the predictive power, if it adds even a tiny bit it will be judged as better

-   "A distinguishing feature of modeling for prediction is the need to test predictive models "out of sample": using independent data that were not used to fit the model." pg 5 - I'm not sure that I actually have this. May want to talk to Mark and/or Cecilia about it

-   Important to note that this paper is mostly relevant to Frequentist statistics; a strict Bayesian wouldn't separate inference, exploration and prediction in the same way
